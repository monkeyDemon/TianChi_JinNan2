# -*- coding: utf-8 -*-
"""
Created on Thu Dec 27 11:22:47 2018

DenseNet models base on Keras.
...
parser = argparse.ArgumentParser(description = 'manual to this script', 
                ...
                command example:
                python %(prog)s --train_set_path='/media/homework/c4b7dd6d-5a63-40d3-b57e-2c870c014235/DATASET_v3_totrain/train/' --valid_set_path='/media/homework/c4b7dd6d-5a63-40d3-b57e-2c870c014235/DATASET_v3_totrain/valid/' '''),
                formatter_class = argparse.RawTextHelpFormatter)
parser.add_argument('--weight_load_path', type = str, 
                default = './weight/densenet121_weights_tf_dim_ordering_tf_kernels_notop.h5',
                help = textwrap.dedent('''\
                path of the weights file pretrained on imageNet
...
parser.add_argument('--weight_save_path', type = str, 
                default = './weight/densenet121_weights_tf_transfor.h5',
                help = textwrap.dedent('''\
                the path to save the weights after training 
                if the specified file already exists, the program will continue to train on it.'''))
parser.add_argument('--train_set_path', type = str, default = None,
                help = 'the root path of the training set. Each category should correspond to a folder ')
parser.add_argument('--valid_set_path', type = str, default = None,
                help = 'the root path of the validation set. Each category should correspond to a folder')
parser.add_argument('--record_save_dir', type = str, default = './record/',
                help = 'the directory to save the tensorBoard training record file')
parser.add_argument('--category_num', type = int, default = 3,
                help = 'the category num of the classification problem, default is 3')
parser.add_argument('--batch_size', type = int, default = 32,
                help = 'size of each batch, default is 32')
...
# record_save_dir    the directory to save the training record file
...
record_save_dir = args.record_save_dir
...
reduceLRcallback = ReduceLROnPlateau(monitor='val_acc', factor=0.3, patience=4,
...
if os.path.exists(weight_save_path):
    ...
    callbacks=[TensorBoard(log_dir=record_save_dir), early_stopping, history, checkpoint, reduceLRcallback]
    )
    sys.exit()
...
# load preTrained denseNet model without top fully connection layer
if os.path.exists(weight_load_path) == False:
	weight_load_path = 'imagenet'
    
baseModel = DenseNet121(weights = weight_load_path, include_top=False, pooling=None, input_shape=(224, 224, 3))
